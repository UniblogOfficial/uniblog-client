import { SocialNetwork, SocialService } from 'common/constants';
import {
  MLDraftAudio,
  MLDraftButton,
  MLDraftCarousel,
  MLDraftDivider,
  MLDraftImage,
  MLDraftImageText,
  MLDraftLink,
  MLDraftLogo,
  MLDraftShop,
  MLDraftSocial,
  MLDraftText,
  MLDraftVideo,
  MLDraftVote,
  MLDraftWidget,
} from 'common/types/instance';

const img2 =
  'data:image/jpeg;base64,';
const img1 =
  'data:image/jpeg;base64,';
const sliderImage1 = 'https://i.ibb.co/xq8q4H3/slider-Image1.png';
const sliderImage2 = 'https://i.ibb.co/8DWmTRm/slider-Image2.png';
const sliderImage3 = 'https://i.ibb.co/W0PwZMy/background-9999-0.png';
export const drafts = [
  {
    name: 'shkolaPifagora',
    background: '#0000',
    outerBackground: '#0000',
    maxWidth: 480,
    blocks: [
      new MLDraftLogo({
        isFilled: true,
        logo: null,
        size: 100,
        margin: [0, 0, 0, 0],
      }),
      new MLDraftText({
        isFilled: true,
        text: 'Привет, меня зовут Евгений, и я готовлю к ЕГЭ и ОГЭ по математике более 10 лет',
        color: '#fff',
        fontSize: 14,
        fontWeight: 400,
        fontFamily: 'Inter, sans-serif',
        textAlign: 'left',
        margin: [0, 0, 24, 0],
        padding: [24, 24, 24, 24],
        background: 'rgba(47, 58, 244, 0.73)',
      }),
      new MLDraftText({
        isFilled: true,
        text: 'ТОП БЕСПЛАТНЫХ КУРСОВ',
        color: '#fff',
        fontSize: 20,
        fontWeight: 700,
        fontFamily: 'Inter, sans-serif',
        textAlign: 'left',
        padding: [12, 24, 12, 24],
        margin: [0, 0, 12, 0],
        background: 'rgba(47, 58, 244, 0.73)',
      }),
      new MLDraftVideo({
        isFilled: true,
        url: 'https://www.youtube.com/embed/hYu0d-4SX_Y',
        width: 327,
        height: 184,
        margin: [0, 0, 24, 0],
        padding: [0, 24, 0, 24],
      }),
      new MLDraftText({
        isFilled: true,
        text: 'ТОП КУРСОВ',
        color: '#fff',
        fontSize: 20,
        fontWeight: 700,
        fontFamily: 'Inter, sans-serif',
        textAlign: 'left',
        padding: [12, 24, 12, 24],
        margin: [0, 0, 12, 0],
        background: 'rgba(47, 58, 244, 0.73)',
      }),
      new MLDraftShop({
        isFilled: true,
        grid: '1fr 1fr 1fr',
        gap: 5,
        cells: [
          {
            order: 0,
            image: null,
            background: '#fff',
            title: 'Курс "ВСПОМНИТЬ ВСЁ" 2022 (повторение всех заданий ЕГЭ)',
            subtitle: '2 490 ₽',
            href: `${process.env.REACT_APP_HOST_PRODUCTION}/purchase?t=wwcs9v`,
          },
          {
            order: 1,
            image: null,
            background: '#fff',
            title: 'Видеокурс 1-11 задания ЕГЭ 2022 профиль (первая часть с нуля)',
            subtitle: '990 ₽',
            href: `${process.env.REACT_APP_HOST_PRODUCTION}/purchase?t=wwcs9v`,
          },
          {
            order: 2,
            image: null,
            background: '#fff',
            title:
              'Видеокурс 12 задания ЕГЭ 2022 профиль (тригонометрия и другие типы 12-х с нуля)',
            subtitle: '790 ₽',
            href: `${process.env.REACT_APP_HOST_PRODUCTION}/purchase?t=wwcs9v`,
          },
          {
            order: 3,
            image: null,
            background: '#fff',
            title: 'Видеокурс 13 задания ЕГЭ 2022 профиль (стереометрия с нуля)',
            subtitle: '790 ₽',
            href: `${process.env.REACT_APP_HOST_PRODUCTION}/purchase?t=wwcs9v`,
          },
          {
            order: 4,
            image: null,
            background: '#fff',
            title: 'Видеокурс 10 задания ЕГЭ 2022 профиль (неравенства с нуля)',
            subtitle: '790 ₽',
            href: `${process.env.REACT_APP_HOST_PRODUCTION}/purchase?t=wwcs9v`,
          },
          {
            order: 5,
            image: null,
            background: '#fff',
            title: 'Видеокурс 16 задания ЕГЭ 2022 профиль (планиметрия с нуля)',
            subtitle: '990 ₽',
            href: `${process.env.REACT_APP_HOST_PRODUCTION}/purchase?t=wwcs9v`,
          },
          {
            order: 6,
            image: null,
            background: '#fff',
            title: 'Видеокурс 15 задания ЕГЭ 2022 профиль (кредиты и другие виды 15-х с нуля)',
            subtitle: '790 ₽',
            href: `${process.env.REACT_APP_HOST_PRODUCTION}/purchase?t=wwcs9v`,
          },
          {
            order: 7,
            image: null,
            background: '#fff',
            title: 'Видеокурс 17 задания ЕГЭ 2022 профиль (параметры с нуля)',
            subtitle: '990 ₽',
            href: `${process.env.REACT_APP_HOST_PRODUCTION}/purchase?t=wwcs9v`,
          },
        ],
        color: '#000',
        fontSize: 10,
        fontWeight: 400,
        fontFamily: 'Inter, sans-serif',
        textAlign: 'left',
        subtitleColor: '#000',
        subtitleFont: '700 12px Inter, sans-serif',
        subtitleTextAlign: 'center',
        margin: [0, 0, 24, 0],
        padding: [12, 24, 12, 24],
      }),
      new MLDraftImage({
        isFilled: true,
        image: null,
        href: 'https://www.youtube.com/c/pifagor1/live',
        imgPosition: 'bottom',
        textPosition: 'outside',
        margin: [0, 0, 24, 0],
        padding: [12, 48, 12, 48],
      }),
      new MLDraftSocial({
        isFilled: true,
        links: [
          'https://vk.com/shkolapifagora',
          'https://www.youtube.com/channel/UC7Z3j8Cgsa2NnBmKAl7uOuQ',
          'https://instagram.com/shkola_pifagora',
        ],
        linkTypes: [SocialNetwork.VK, SocialNetwork.YOUTUBE, SocialNetwork.INSTAGRAM],
        size: 75,
        margin: [0, 0, 24, 0],
        padding: [12, 24, 12, 24],
      }),
      new MLDraftLink({
        isFilled: false,
        href: '#',
        linkType: 'third-party' as const,
        title: 'Только наши каналы',
        fontSize: 20,
        fontWeight: 700,
        fontFamily: 'Inter, sans-serif',
        textShadow: ['2px 1px 2px #0009'],
        textAlign: 'center',
        color: '#fff',
        background: 'rgba(47, 58, 244, 0.73)',
        borderRadius: [5, 5, 5, 5],
        margin: [0, 12, 12, 12],
        padding: [12, 24, 12, 24],
      }),
      new MLDraftLink({
        isFilled: false,
        href: 'https://www.youtube.com/c/pifagor1/live',
        linkType: 'third-party' as const,
        title: 'Группа ЕГЭ',
        fontSize: 20,
        fontWeight: 700,
        fontFamily: 'Inter, sans-serif',
        textShadow: ['2px 1px 2px #0009'],
        textAlign: 'center',
        color: '#fff',
        background: 'rgba(47, 58, 244, 0.73)',
        borderRadius: [5, 5, 5, 5],
        margin: [0, 12, 12, 12],
        padding: [12, 24, 12, 24],
      }),
      new MLDraftLink({
        isFilled: false,
        href: 'https://www.youtube.com/user/ShkolaPifagora',
        linkType: 'third-party' as const,
        title: 'Группа ОГЭ',
        fontSize: 20,
        fontWeight: 700,
        fontFamily: 'Inter, sans-serif',
        textShadow: ['2px 1px 2px #0009'],
        textAlign: 'center',
        color: '#fff',
        background: 'rgba(47, 58, 244, 0.73)',
        borderRadius: [5, 5, 5, 5],
        margin: [0, 12, 12, 12],
        padding: [12, 24, 12, 24],
      }),
    ],
  },
  {
    name: 'pub13_menu',
    background: '#1F1A1A',
    outerBackground: 'rgba(31, 26, 26, 1)',
    maxWidth: 480,
    blocks: [
      new MLDraftLogo({
        logo: 'https://i.ibb.co/z4McDWc/logo-0-1.png',
        isFilled: true,
        size: 100,
      }),
      new MLDraftText({
        isFilled: false,
        text: 'ПАБ XIII | ПАБ 13 Тольятти',
        textAlign: 'center',
        color: 'rgba(255, 255, 255, 1)',
        fontWeight: 700,
        fontSize: 22,
        margin: [0, 0, 24, 0],
        padding: [12, 24, 12, 24],
      }),
      new MLDraftText({
        isFilled: false,
        text: 'Каждый день с 11:30 до 16:00 скидка 25% на все меню + суп дня БЕСПЛАТНО.\n\nСписок наших акций:\n\nПН Скидка 30%/40%* на стейки!\n\nВТ Скидка 30%/40%* на закуски!\n\nСР Скидка женским компаниям от двух человек 30%/40%* на бар или 30%/40%* на кухню!\n\nЧТ Пиво с крана и настойки по акции 1+1!\n\nПТ и СБ Угощаем настойками!\n\nВС Скидка на бургеры и стритфуд 30%/40%*!',
        textAlign: 'left',
        color: 'rgba(255, 255, 255, 1)',
        fontWeight: 400,
        fontSize: 18,
        lineHeight: 1,
        fontFamily: 'Inter',
        letterSpacing: -1.2,
        margin: [0, 0, 24, 0],
        padding: [0, 24, 0, 24],
      }),
      new MLDraftSocial({
        isFilled: false,
        links: [
          'https://vk.com/xiii_pub',
          'https://t.me/xiiipub',
          'https://www.instagram.com/xiiipub',
        ],
        linkTypes: [SocialNetwork.VK, SocialNetwork.TELEGRAM, SocialNetwork.INSTAGRAM],
        size: 65,
        margin: [0, 0, 60, 0],
        padding: [12, 12, 12, 12],
      }),
      new MLDraftImage({
        isFilled: true,
        image: 'https://i.ibb.co/XCH56vQ/image-0-0.png',
        padding: [0, 24, 0, 24],
        imgPosition: 'bottom',
        textPosition: 'outside',
      }),
      new MLDraftImage({
        isFilled: true,
        image: 'https://i.ibb.co/zFcRFbR/image-0-0.png',
        padding: [0, 24, 0, 24],
        imgPosition: 'bottom',
        textPosition: 'outside',
      }),
      new MLDraftImage({
        isFilled: true,
        image: 'https://i.ibb.co/GkFMVFs/image-0-0.png',
        padding: [0, 24, 0, 24],
        imgPosition: 'bottom',
        textPosition: 'outside',
      }),
      new MLDraftImage({
        isFilled: true,
        image: 'https://i.ibb.co/88tyNM5/image-0-0.png',
        padding: [0, 24, 0, 24],
        imgPosition: 'bottom',
        textPosition: 'outside',
      }),
      new MLDraftImage({
        isFilled: true,
        image: 'https://i.ibb.co/L9GQMMV/image-0-0.png',
        padding: [0, 24, 0, 24],
        imgPosition: 'bottom',
        textPosition: 'outside',
      }),
      new MLDraftImage({
        isFilled: true,
        image: 'https://i.ibb.co/ts3yDnX/image-0-0.png',
        padding: [0, 24, 0, 24],
        imgPosition: 'bottom',
        textPosition: 'outside',
      }),
      new MLDraftImage({
        isFilled: true,
        image: 'https://i.ibb.co/wpmkTJx/image-0-0.png',
        padding: [0, 24, 0, 24],
        imgPosition: 'bottom',
        textPosition: 'outside',
      }),
      new MLDraftImage({
        isFilled: true,
        image: 'https://i.ibb.co/tBPLC6v/image-0-0.png',
        padding: [0, 24, 0, 24],
        imgPosition: 'bottom',
        textPosition: 'outside',
      }),
      new MLDraftImage({
        isFilled: true,
        image: 'https://i.ibb.co/dmz5DKq/image-0-0.png',
        padding: [0, 24, 0, 24],
        imgPosition: 'bottom',
        textPosition: 'outside',
      }),
    ],
  },
  {
    name: 'lex',
    background: '#0000',
    outerBackground: '#0000',
    maxWidth: 480,
    blocks: [
      new MLDraftLogo({
        isFilled: true,
        logo: null,
        size: 100,
        margin: [0, 0, 0, 0],
      }),
      new MLDraftText({
        isFilled: false,
        text: 'Lex',
        fontSize: 28,
        fontWeight: 700,
        fontFamily: 'Inter, sans-serif',
        textShadow: ['2px 1px 2px #0009'],
        textAlign: 'center',
        color: '#fff',
        margin: [0, 0, 24, 0],
        padding: [0, 40, 0, 40],
      }),
      new MLDraftText({
        isFilled: false,
        text: 'Болею за «Арсенал» уже больше 20 лет. Пишу про свои мысли про клуб, и не только. Веду паблик в вк, знаю о новостях «Арсенала» практически все. ',
        fontSize: 28,
        fontWeight: 700,
        fontFamily: 'Inter, sans-serif',
        textShadow: ['2px 1px 2px #0009'],
        textAlign: 'center',
        color: '#fff',
        margin: [0, 0, 60, 0],
        padding: [0, 40, 0, 40],
      }),
      new MLDraftSocial({
        isFilled: false,
        links: [
          'https://vk.com/arsscnews',
          'https://www.youtube.com/channel/UCBYo2XB0I9Q-EMdear8fD5w',
          'https://t.me/',
          'https://www.arsenal.com/',
        ],
        linkTypes: [
          SocialNetwork.VK,
          SocialNetwork.YOUTUBE,
          SocialNetwork.TELEGRAM,
          SocialNetwork.INSTAGRAM,
        ],
        size: 65,
        margin: [0, 0, 60, 0],
        padding: [12, 12, 12, 12],
      }),
      new MLDraftText({
        isFilled: false,
        text: 'Поддержка через Donation:',
        fontSize: 20,
        fontWeight: 400,
        fontFamily: 'Inter, sans-serif',
        textAlign: 'center',
        color: '#fff',
        margin: [0, 0, 12, 0],
        padding: [0, 24, 0, 24],
      }),
      new MLDraftImage({
        isFilled: true,
        image: null,
        imgPosition: 'bottom',
        textPosition: 'outside',
        margin: [0, 0, 6, 0],
        padding: [0, 92, 0, 92],
      }),
      new MLDraftText({
        isFilled: false,
        text: 'Donation Alerts',
        fontSize: 18,
        fontWeight: 400,
        textAlign: 'center',
        color: '#fff',
        margin: [0, 0, 24, 0],
        padding: [0, 24, 0, 24],
      }),
    ],
  },
  {
    name: 'evgenykrasikov',
    background: 'url(https://i.ibb.co/k6wzJxQ/background-9999-0.png)',
    outerBackground: 'url(https://i.ibb.co/k6wzJxQ/background-9999-0.png)',
    maxWidth: 480,
    blocks: [
      new MLDraftLogo({
        isFilled: true,
        logo: 'https://i.ibb.co/WPsJK4R/image-0-0.png',
        size: 175,
        padding: [40, 0, 40, 0],
      }),
      /* new MLDraftImage({
        isFilled: true,
        image: 'https://i.ibb.co/WPsJK4R/image-0-0.png',
        imgPosition: 'bottom',
        textPosition: 'outside',
        margin: [0, 0, 0, 0],
        padding: [40, 80, 60, 0],
      }), */
      new MLDraftText({
        isFilled: false,
        text: 'Eвгений',
        fontWeight: 700,
        fontSize: 26,
        fontFamily: 'Inter, sans-serif',
        textAlign: 'center',
        color: '#fff',
        textShadow: ['2px 1px 2px #0009'],
        background: 'rgba(47, 58, 244, 0.73)',
        margin: [0, 0, 0, 0],
        padding: [12, 24, 0, 24],
      }),
      new MLDraftText({
        isFilled: false,
        text: 'Основатель сервиса',
        fontWeight: 400,
        fontSize: 18,
        fontFamily: 'Inter, sans-serif',
        textAlign: 'center',
        color: '#fff',
        background: 'rgba(47, 58, 244, 0.73)',
        margin: [0, 0, 24, 0],
        padding: [0, 24, 12, 24],
      }),
      new MLDraftLink({
        isFilled: false,
        href: 'https://uniblog.online',
        linkType: 'third-party' as const,
        title: 'Материалы о проекте',
        fontWeight: 400,
        fontSize: 22,
        fontFamily: 'Inter, sans-serif',
        textAlign: 'center',
        color: '#fff',
        textShadow: ['2px 1px 2px #0009'],
        background: 'rgba(47, 58, 244, 0.73)',
        margin: [0, 0, 24, 0],
        padding: [12, 24, 12, 24],
      }),
      new MLDraftLink({
        isFilled: false,
        href: 'https://linkii.ru',
        linkType: 'third-party' as const,
        title: 'Наш классический сайт',
        fontWeight: 400,
        fontSize: 22,
        fontFamily: 'Inter, sans-serif',
        textShadow: ['2px 1px 2px #0009'],
        textAlign: 'center',
        color: '#fff',
        background: 'rgba(47, 58, 244, 0.73)',
        margin: [0, 0, 0, 0],
        padding: [12, 24, 12, 24],
      }),
      new MLDraftImage({
        isFilled: true,
        image: 'https://i.ibb.co/XZj3w7g/image-0-0.png',
        imgPosition: 'bottom',
        textPosition: 'outside',
        margin: [0, 0, 12, 0],
        padding: [0, 16, 0, 16],
      }),
      new MLDraftSocial({
        isFilled: true,
        links: [
          'https://vk.com/kracko23',
          'https://discordapp.com/users/534986739513819138/',
          'https://tlgg.ru/Evgeny163',
          'https://msng.link/o/?krasikov.evgeny=ig',
        ],
        linkTypes: [
          SocialNetwork.VK,
          SocialService.DISCORD,
          SocialNetwork.TELEGRAM,
          SocialNetwork.INSTAGRAM,
        ],
        size: 65,
        margin: [0, 0, 60, 0],
        padding: [12, 12, 12, 12],
      }),
    ],
  },
  {
    name: 'pub13',
    background: 'rgb(13, 13, 13)',
    outerBackground: 'rgb(13, 13, 13)',
    maxWidth: 480,
    blocks: [
      new MLDraftLogo({
        isFilled: true,
        logo: 'https://i.ibb.co/mzG4W7P/logo-0-1.png',
        banner: 'https://i.ibb.co/MSB0DkH/logo-0-2.png',
        size: 175,
        padding: [56, 0, 36, 0],
      }),
      new MLDraftButton({
        isFilled: false,
        href: '',
        title: 'Забронировать столик',
        fontSize: 24,
        fontWeight: 400,
        fontFamily: 'Inter, sans-serif',
        letterSpacing: 1,
        textShadow: ['2px 1px 2px #0009'],
        padding: [18, 24, 18, 24],
        margin: [0, 0, 24, 0],
        background: '#E8A23999',
      }),
      new MLDraftButton({
        isFilled: false,
        href: '',
        title: 'Заказать доставку',
        fontSize: 24,
        fontWeight: 400,
        fontFamily: 'Inter, sans-serif',
        letterSpacing: 1,
        textShadow: ['2px 1px 2px #0009'],
        padding: [18, 24, 18, 24],
        margin: [0, 0, 24, 0],
        background: '#E8A23999',
      }),
      new MLDraftText({
        isFilled: false,
        text: 'Контакты',
        fontSize: 32,
        fontWeight: 700,
        fontFamily: 'Inter, sans-serif',
        textAlign: 'left',
        color: '#fff',
        margin: [0, 0, 0, 0],
        padding: [12, 24, 6, 24],
      }),
      new MLDraftText({
        isFilled: false,
        text: 'Адрес',
        fontSize: 24,
        fontWeight: 400,
        fontFamily: 'Inter, sans-serif',
        textAlign: 'left',
        color: '#fff',
        margin: [0, 0, 0, 0],
        padding: [6, 24, 6, 24],
      }),
      new MLDraftText({
        isFilled: false,
        text: 'Советская ул., 74А, Тольятти',
        fontSize: 16,
        fontWeight: 400,
        fontFamily: 'Inter, sans-serif',
        textAlign: 'left',
        color: '#fff',
        margin: [0, 0, 0, 0],
        padding: [6, 24, 6, 24],
      }),
      new MLDraftText({
        isFilled: false,
        text: 'Время работы:',
        fontSize: 24,
        fontWeight: 400,
        fontFamily: 'Inter, sans-serif',
        textAlign: 'left',
        color: '#fff',
        margin: [0, 0, 0, 0],
        padding: [6, 24, 6, 24],
      }),
      new MLDraftText({
        isFilled: false,
        text: 'Вс-Чт: 11.30-24.00\n\nПт-Сб: 11.30-2.00',
        fontSize: 16,
        fontWeight: 400,
        fontFamily: 'Inter, sans-serif',
        textAlign: 'left',
        color: '#fff',
        margin: [0, 0, 0, 0],
        padding: [6, 24, 6, 24],
      }),
      new MLDraftText({
        isFilled: false,
        text: 'Телефон',
        fontSize: 24,
        fontWeight: 400,
        fontFamily: 'Inter, sans-serif',
        textAlign: 'left',
        color: '#fff',
        margin: [0, 0, 0, 0],
        padding: [6, 24, 6, 24],
      }),
      new MLDraftText({
        isFilled: false,
        text: '+7 (848) 239-46-13',
        fontSize: 16,
        fontWeight: 400,
        fontFamily: 'Inter, sans-serif',
        textAlign: 'left',
        color: '#fff',
        margin: [0, 0, 0, 0],
        padding: [6, 24, 12, 24],
      }),
      new MLDraftVote({
        isFilled: false,
        cells: [
          {
            order: 0,
            title: 'Как Вам бар?',
            value: 0,
          },
          {
            order: 1,
            title: 'Как Вам блюда?',
            value: 0,
          },
        ],
        target: 'petricowork@gmail.com',
        titleBackground: '#E8A23999',
        fontSize: 24,
        fontWeight: 400,
        fontFamily: 'Inter, sans-serif',
        textAlign: 'center',
        color: '#fff',
        textShadow: ['2px 1px 2px #0009'],
        buttonBackground: '#E8A23999',
        buttonFontSize: 24,
        buttonFontWeight: 400,
        buttonFontFamily: 'Inter, sans-serif',
        buttonLetterSpacing: 1,
        buttonTextShadow: ['2px 1px 2px #0009'],
        buttonTextAlign: 'center',
        margin: [0, 0, 60, 0],
        padding: [18, 0, 18, 0],
      }),
      new MLDraftSocial({
        isFilled: false,
        links: [
          'https://vk.com/xiii_pub',
          'https://t.me/xiiipub',

          'https://www.instagram.com/xiiipub',
        ],
        linkTypes: [SocialNetwork.VK, SocialNetwork.TELEGRAM, SocialNetwork.INSTAGRAM],
        size: 65,
        margin: [0, 0, 60, 0],
        padding: [12, 12, 12, 12],
      }),
    ],
  },
  {
    name: 'Утюг',
    background: '#0000',
    outerBackground: '#0000',
    maxWidth: 480,
    blocks: [
      new MLDraftLogo({
        isFilled: true,
        logo: null,
        size: 154,
        padding: [56, 0, 36, 0],
      }),
      new MLDraftText({
        isFilled: false,
        text: 'Руслан Утюг',
        fontSize: 28,
        fontWeight: 400,
        fontFamily: 'Inter, sans-serif',
        lineHeight: 1.4,
        color: '#fff',
        textAlign: 'center',
        margin: [0, 0, 0, 0],
        padding: [12, 24, 12, 24],
        background: '#E839CC99',
      }),
      new MLDraftText({
        isFilled: false,
        text: 'новая песня на площадках',
        fontSize: 18,
        fontWeight: 400,
        fontFamily: 'Inter, sans-serif',
        textAlign: 'center',
        color: '#fff',
        margin: [0, 0, 0, 0],
        padding: [12, 24, 12, 24],
        background: '#E839CC99',
      }),
      new MLDraftSocial({
        isFilled: false,
        links: ['https://vk.com', 'https://youtube.com', 'https://t.me', 'https://instagram.com'],
        linkTypes: [
          SocialNetwork.VK,
          SocialNetwork.YOUTUBE,
          SocialNetwork.TELEGRAM,
          SocialNetwork.INSTAGRAM,
        ],
        size: 65,
        margin: [39, 0, 33, 0],
        padding: [0, 17, 0, 17],
      }),
      new MLDraftSocial({
        isFilled: false,
        links: [
          'https://www.tiktok.com/',
          'https://www.twitch.tv/',
          'https://boosty.to/',
          'https://mail.google.com/mail/',
        ],
        linkTypes: [
          SocialNetwork.TIKTOK,
          SocialService.TWITCH,
          SocialService.BOOSTY,
          SocialService.GMAIL,
        ],
        size: 65,
        margin: [0, 0, 0, 0],
        padding: [0, 17, 0, 17],
      }),
      new MLDraftImage({
        isFilled: true,
        image: null,
        imgPosition: 'bottom',
        textPosition: 'inside',
        margin: [20, 0, 0, 0],
        padding: [0, 0, 0, 0],
      }),
      new MLDraftImage({
        isFilled: true,
        image: null,
        imgPosition: 'bottom',
        textPosition: 'inside',
        margin: [20, 0, 0, 0],
        padding: [0, 0, 0, 0],
      }),
      new MLDraftLink({
        isFilled: false,
        href: 'https://boom.ru/',
        linkType: 'third-party' as const,
        title: 'Слушай в ВООМ',
        textAlign: 'center',
        fontSize: 24,
        fontWeight: 400,
        fontFamily: 'Inter, sans-serif',
        color: '#fff',
        padding: [24, 24, 24, 24],
        margin: [0, 24, 12, 24],
        borderRadius: [15, 15, 15, 15],
        background: '#E839CC99',
      }),
      new MLDraftLink({
        isFilled: false,
        href: 'https://vk.com/audio',
        linkType: 'third-party' as const,
        title: 'Слушай в VK',
        textAlign: 'center',
        fontSize: 24,
        fontWeight: 400,
        fontFamily: 'Inter, sans-serif',
        color: '#fff',
        padding: [24, 24, 24, 24],
        margin: [0, 24, 12, 24],
        borderRadius: [15, 15, 15, 15],
        background: '#E839CC99',
      }),
      new MLDraftLink({
        isFilled: false,
        href: 'https://open.spotify.com/',
        linkType: 'third-party' as const,
        title: 'Слушай в Spotify',
        textAlign: 'center',
        fontSize: 24,
        fontWeight: 400,
        fontFamily: 'Inter, sans-serif',
        color: '#fff',
        padding: [24, 24, 24, 24],
        margin: [0, 24, 12, 24],
        borderRadius: [15, 15, 15, 15],
        background: '#E839CC99',
      }),
      new MLDraftLink({
        isFilled: false,
        href: 'https://music.yandex.by/home',
        linkType: 'third-party' as const,
        title: 'Слушай в Яндекс',
        textAlign: 'center',
        fontSize: 24,
        fontWeight: 400,
        fontFamily: 'Inter, sans-serif',
        color: '#FFFBFB',
        padding: [24, 24, 24, 24],
        margin: [0, 24, 12, 24],
        borderRadius: [50, 15, 15, 50],
        background: '#E839CC99',
      }),
      new MLDraftSocial({
        isFilled: false,
        links: [
          'https://vk.com',
          'https://youtube.com',
          'https://www.tiktok.com/',
          'https://instagram.com',
        ],
        linkTypes: [
          SocialNetwork.VK,
          SocialNetwork.YOUTUBE,
          SocialNetwork.TIKTOK,
          SocialNetwork.INSTAGRAM,
        ],
        size: 65,
        margin: [55, 55, 55, 55],
        padding: [0, 17, 0, 17],
      }),
    ],
  },
  {
    name: 'LesaFS',
    background: '#0000',
    outerBackground: '#0000',
    maxWidth: 480,
    blocks: [
      // LesaFS
      /* new MLDraftLogo({
        isFilled: !!avatar,
        logo,
        size: 154,
        padding: [56, 0, 36],
      }), */
      new MLDraftImage({
        isFilled: false,
        image: 'https://i.ibb.co/3SY8kWH/image-0-0.png',
        imgPosition: 'bottom',
        textPosition: 'outside',
        padding: [56, 56, 40, 56],
      }),
      new MLDraftText({
        isFilled: false,
        text: 'Lesa FS',
        fontWeight: 400,
        fontSize: 24,
        fontFamily: 'Inter, sans-serif',
        color: '#fff',
        textAlign: 'center',
        textShadow: ['1px 1px 2px #0003'],
        margin: [0, 0, 0, 0],
        padding: [38.5, 24, 0, 0],
        background: '#E839CC99',
      }),
      new MLDraftImage({
        isFilled: true,
        image: null,
        href: 'https://share.boom.ru/album/20152494/?share_auth=023985e6644b5c9a66811a629924d0',
        imgPosition: 'bottom',
        textPosition: 'outside',
        margin: [12, 24, 12, 24],
      }),
      new MLDraftImage({
        isFilled: true,
        image: null,
        href: 'https://music.yandex.ru/album/22583577',
        imgPosition: 'bottom',
        textPosition: 'outside',
        margin: [12, 55, 12, 55],
      }),
      new MLDraftSocial({
        isFilled: false,
        links: [
          'https://vk.com/lesafs',
          'https://vk.com/lesa_fs',
          'https://t.me/lesa_fs_music',
          'https://instagram.com/lesa_fs',
        ],
        linkTypes: [
          SocialNetwork.VK,
          SocialNetwork.VK,
          SocialNetwork.TELEGRAM,
          SocialNetwork.INSTAGRAM,
        ],
        size: 65,
        margin: [0, 0, 0, 0],
        padding: [0, 17, 0, 17],
      }),
    ],
  },
  {
    name: 'CatsBackground',
    background: '#0000',
    outerBackground: '#0000',
    maxWidth: 480,
    blocks: [
      // CatsBackground
      new MLDraftLogo({
        isFilled: true,
        logo: null,
        size: 154,
        padding: [5, 0, 0, 0],
        margin: [0, 0, 24, 0],
      }),
      new MLDraftButton({
        isFilled: false,
        href: '',
        title: 'Стрижка - 500р',
        fontSize: 24,
        fontWeight: 400,
        fontFamily: 'Inter, sans-serif',
        color: '#fff',
        letterSpacing: 1,
        textShadow: ['2px 1px 2px #0009'],
        padding: [20, 24, 20, 24],
        borderRadius: [15, 15, 15, 15],
        margin: [0, 0, 12, 0],
        background: 'rgba(232,57,204,0.6)',
      }),
      new MLDraftButton({
        isFilled: false,
        href: '',
        title: 'Записаться на стрижку',
        fontSize: 24,
        fontWeight: 400,
        fontFamily: 'Inter, sans-serif',
        color: '#fff',
        letterSpacing: 1,
        textShadow: ['2px 1px 2px #0009'],
        padding: [20, 24, 20, 24],
        borderRadius: [15, 15, 15, 15],
        margin: [0, 0, 15, 0],
        background: 'rgba(232,57,204,0.6)',
      }),
      new MLDraftWidget({
        isFilled: true,
        url: 'https://widget.qiwi.com/widgets/middle-widget-300x300?publicKey=48e7qUxn9T7RyYE1MVZswX1FRSbE6iyCj2gCRwwF3Dnh5XrasNTx3BGPiMsyXQFNKQhvukniQG8RTVhYm3iPsXMCEHsEfvcM9fWDrHZGpWKAFeZPjiJbFtPjyxAanWqYMcU9DSN7wMpc8VGAVjmd9yjdSQRA92xGdkxFbtNyRSzFPrYeoktJhC8rNxvbe',
        width: 300,
        height: 300,
        margin: [0, 0, 12, 0],
        padding: [0, 12, 0, 12],
      }),
      new MLDraftSocial({
        isFilled: false,
        links: [
          'https://vk.com',
          'https://t.me',
          'https://instagram.com',
          'https://www.whatsapp.com/',
        ],
        linkTypes: [
          SocialNetwork.VK,
          SocialNetwork.TELEGRAM,
          SocialService.WHATSAPP,
          SocialNetwork.INSTAGRAM,
        ],
        size: 65,
        margin: [0, 0, 0, 0],
        padding: [0, 17, 0, 17],
      }),
    ],
  },
  {
    name: 'rescueFund',
    background: '#0000',
    outerBackground: '#0000',
    maxWidth: 480,
    blocks: [
      // rescueFund
      new MLDraftLogo({
        isFilled: true,
        logo: null,
        size: 154,
        margin: [0, 0, 24, 0],
      }),
      new MLDraftText({
        isFilled: false,
        text: 'Фонд "Спасение"',
        fontSize: 24,
        fontWeight: 400,
        fontFamily: 'Inter, sans-serif',
        color: '#fff',
        textAlign: 'center',
        margin: [0, 0, 16, 0],
        padding: [20, 24, 20, 24],
        background: 'rgba(57,95,232,0.6)',
      }),
      new MLDraftButton({
        isFilled: false,
        href: '',
        title: 'Оплатить',
        fontSize: 24,
        fontWeight: 400,
        fontFamily: 'Inter, sans-serif',
        color: '#fff',
        letterSpacing: 1,
        textShadow: ['2px 1px 2px #0009'],
        padding: [15, 24, 15, 24],
        borderRadius: [51, 51, 51, 51],
        margin: [0, 21, 15, 21],
        background: 'rgb(49,255,0)',
      }),
      new MLDraftImage({
        isFilled: true,
        image: null,
        imgPosition: 'bottom',
        textPosition: 'outside',
        margin: [12, 24, 12, 24],
        padding: [0, 16, 5, 0],
      }),
      new MLDraftSocial({
        isFilled: false,
        links: [
          'https://vk.com',
          'https://t.me',
          'https://www.patreon.com/',
          'https://instagram.com',
        ],
        linkTypes: [
          SocialNetwork.VK,
          SocialService.BOOSTY,
          SocialService.PATREON,
          SocialNetwork.INSTAGRAM,
        ],
        size: 65,
        margin: [0, 0, 0, 0],
        padding: [0, 17, 0, 17],
      }),
    ],
  },
  {
    name: 'vikinG',
    background: '#0000',
    outerBackground: '#0000',
    maxWidth: 480,
    blocks: [
      // vikinG
      new MLDraftLogo({
        isFilled: true,
        logo: null,
        size: 154,
      }),
      new MLDraftText({
        isFilled: false,
        text: 'VikinG Обучение IТату в Тольятти',
        fontSize: 24,
        fontWeight: 400,
        fontFamily: 'Inter, sans-serif',
        color: '#fff',
        textAlign: 'center',
        margin: [0, 0, 16, 0],
        padding: [20, 24, 20, 24],
        background: '#E8394E99',
      }),
      new MLDraftImage({
        isFilled: true,
        image: null,
        imgPosition: 'bottom',
        textPosition: 'inside',
        margin: [12, 24, 12, 24],
        padding: [20, 16, 20, 16],
      }),
      new MLDraftSocial({
        isFilled: false,
        links: ['https://vk.com', 'https://youtube.com', 'https://t.me', 'https://instagram.com'],
        linkTypes: [
          SocialNetwork.VK,
          SocialNetwork.YOUTUBE,
          SocialNetwork.TELEGRAM,
          SocialNetwork.INSTAGRAM,
        ],
        size: 65,
        margin: [0, 0, 0, 0],
        padding: [0, 17, 0, 17],
      }),
    ],
  },
  {
    name: 'lesa',
    background: '#0000',
    outerBackground: '#0000',
    maxWidth: 480,
    blocks: [
      // lesa
      new MLDraftImage({
        isFilled: false,
        image: 'https://i.ibb.co/3SY8kWH/image-0-0.png',
        imgPosition: 'bottom',
        textPosition: 'outside',
        padding: [56, 56, 40, 56],
      }),
      new MLDraftLink({
        isFilled: false,
        href: 'https://music.apple.com/ru/album/1630423033?at=1000lvHr',
        linkType: 'third-party' as const,
        title: 'Apple Music',
        textAlign: 'center',
        image: 'https://i.ibb.co/0tz1PNj/link-0-1.png',
        fontSize: 24,
        fontFamily: 'Inter, sans-serif',
        color: '#fff',
        textShadow: ['2px 1px 2px #0005'],
        padding: [24, 20, 24, 80],
        margin: [0, 24, 32, 24],
        borderRadius: [50, 15, 15, 50],
        background: 'rgba(232,57,204,0.6)',
      }),
      new MLDraftLink({
        isFilled: false,
        href: 'https://music.yandex.ru/album/22778461',
        linkType: 'third-party' as const,
        title: 'Слушай в Яндекс',
        textAlign: 'center',
        image: 'https://i.ibb.co/JQzhk1r/link-0-1.png',
        fontSize: 24,
        fontFamily: 'Inter, sans-serif',
        color: '#FFFBFB',
        textShadow: ['2px 1px 2px #0005'],
        padding: [24, 20, 24, 80],
        margin: [0, 24, 32, 24],
        borderRadius: [50, 15, 15, 50],
        background: 'rgba(232,57,204,0.6)',
      }),
      new MLDraftLink({
        isFilled: false,
        href: 'https://share.boom.ru/album/20367095?share_auth=023d3b0f48b141abce811a62be0ed0',
        linkType: 'third-party' as const,
        title: 'Слушай в VK',
        textAlign: 'center',
        image: 'https://i.ibb.co/fk0tGpm/link-0-1.png',
        fontSize: 24,
        fontFamily: 'Inter, sans-serif',
        color: '#fff',
        textShadow: ['2px 1px 2px #0005'],
        padding: [24, 20, 24, 80],
        margin: [0, 24, 32, 24],
        borderRadius: [50, 15, 15, 50],
        background: 'rgba(232,57,204,0.6)',
      }),
      new MLDraftLink({
        isFilled: false,
        href: 'https://accounts.spotify.com/login?continue=https%3A%2F%2Fopen.spotify.com%2Falbum%2F74pr8sjT3iAutMvyGj9OJF',
        linkType: 'third-party' as const,
        title: 'Слушай в Spotify',
        textAlign: 'center',
        image: 'https://i.ibb.co/ZzmKdcF/link-0-1.png',
        fontSize: 24,
        fontFamily: 'Inter, sans-serif',
        color: '#fff',
        textShadow: ['2px 1px 2px #0005'],
        padding: [24, 20, 24, 80],
        margin: [0, 24, 32, 24],
        borderRadius: [50, 15, 15, 50],
        background: 'rgba(232,57,204,0.6)',
      }),
      new MLDraftLink({
        isFilled: false,
        href: 'https://music.apple.com/ru/album/1630423033?at=1000lvHr',
        linkType: 'third-party' as const,
        title: 'В iTunes',
        textAlign: 'center',
        image: 'https://i.ibb.co/QPt83n9/link-0-1.png',
        fontSize: 24,
        fontFamily: 'Inter, sans-serif',
        color: '#fff',
        textShadow: ['2px 1px 2px #0005'],
        padding: [24, 20, 24, 80],
        margin: [0, 24, 32, 24],
        borderRadius: [50, 15, 15, 50],
        background: 'rgba(232,57,204,0.6)',
      }),
      new MLDraftLink({
        isFilled: false,
        href: 'https://zvuk.com/release/24523908',
        linkType: 'third-party' as const,
        title: 'Слушай на Звук',
        textAlign: 'center',
        image: 'https://i.ibb.co/0Zz0x1p/link-0-1.png',
        fontSize: 24,
        fontFamily: 'Inter, sans-serif',
        color: '#fff',
        textShadow: ['2px 1px 2px #0005'],
        padding: [24, 20, 24, 80],
        margin: [0, 24, 32, 24],
        borderRadius: [50, 15, 15, 50],
        background: 'rgba(232,57,204,0.6)',
      }),
      new MLDraftLink({
        isFilled: false,
        href: 'https://music.youtube.com/playlist?list=OLAK5uy_mcLnULIsKyuVo8nG5C7BH2cunCluGShus&feature=share',
        linkType: 'third-party' as const,
        title: 'YouTube Music',
        textAlign: 'center',
        image: 'https://i.ibb.co/1KNqSz3/link-0-1.png',
        fontSize: 24,
        fontFamily: 'Inter, sans-serif',
        color: '#FFFBFB',
        textShadow: ['2px 1px 2px #0005'],
        padding: [24, 20, 24, 80],
        margin: [0, 24, 32, 24],
        borderRadius: [50, 15, 15, 50],
        background: 'rgba(232,57,204,0.6)',
      }),
      new MLDraftLink({
        isFilled: false,
        href: 'https://ok.ru/music/album/122937016505133',
        linkType: 'third-party' as const,
        title: 'Одноклассники',
        textAlign: 'center',
        image: 'https://i.ibb.co/QpKwGq4/link-0-1.png',
        fontSize: 24,
        fontFamily: 'Inter, sans-serif',
        color: '#fff',
        textShadow: ['2px 1px 2px #0005'],
        padding: [24, 20, 24, 80],
        margin: [0, 24, 32, 24],
        borderRadius: [50, 15, 15, 50],
        background: 'rgba(232,57,204,0.6)',
      }),
      new MLDraftSocial({
        isFilled: false,
        links: [
          'https://vk.com/lesafs',
          'https://vk.com/lesa_fs',
          'https://t.me/lesa_fs_music',
          'https://instagram.com/lesa_fs',
        ],
        linkTypes: [
          SocialNetwork.VK,
          SocialNetwork.VK,
          SocialNetwork.TELEGRAM,
          SocialNetwork.INSTAGRAM,
        ],
        size: 67,
        margin: [35, 35, 35, 35],
        padding: [0, 17, 0, 17],
      }),
    ],
  },
  {
    name: 'andreyYurchenkov',
    background: '#0000',
    outerBackground: '#0000',
    maxWidth: 480,
    blocks: [
      // andreyYurchenkov
      new MLDraftLogo({
        isFilled: true,
        logo: null,
        size: 154,
        padding: [56, 0, 36, 0],
      }),
      new MLDraftText({
        isFilled: false,
        text: 'Андрей Юрченков',
        fontSize: 24,
        fontWeight: 400,
        fontFamily: 'Inter, sans-serif',
        color: '#fff',
        textAlign: 'center',
        margin: [0, 0, 0, 0],
        padding: [12, 24, 12, 24],
        background: '#E8394E99',
      }),
      new MLDraftText({
        isFilled: false,
        text: 'Основатель музыкальной онлайн школы "Путь Музыканта"',
        fontSize: 18,
        fontWeight: 400,
        fontFamily: 'Inter, sans-serif',
        textAlign: 'center',
        color: '#fff',
        margin: [0, 0, 0, 0],
        padding: [12, 24, 12, 24],
        background: '#E8394E99',
      }),
      new MLDraftSocial({
        isFilled: false,
        links: ['https://vk.com', 'https://youtube.com', 'https://t.me', 'https://instagram.com'],
        linkTypes: [
          SocialNetwork.VK,
          SocialNetwork.YOUTUBE,
          SocialNetwork.TELEGRAM,
          SocialNetwork.INSTAGRAM,
        ],
        size: 65,
        margin: [39, 0, 33, 0],
        padding: [0, 17, 0, 17],
      }),
      new MLDraftSocial({
        isFilled: false,
        links: [
          'https://www.tiktok.com/',
          'https://www.twitch.tv/',
          'https://boosty.to/',
          'https://mail.google.com/mail/',
        ],
        linkTypes: [
          SocialNetwork.TIKTOK,
          SocialService.TWITCH,
          SocialService.BOOSTY,
          SocialService.GMAIL,
        ],
        size: 65,
        margin: [0, 0, 425, 0],
        padding: [0, 17, 0, 17],
      }),
      new MLDraftLink({
        isFilled: false,
        href: 'https://boom.ru/',
        linkType: 'third-party' as const,
        title: 'Слушай в ВООМ',
        textAlign: 'center',
        fontSize: 24,
        fontWeight: 400,
        fontFamily: 'Inter, sans-serif',
        color: '#fff',
        padding: [24, 24, 24, 24],
        margin: [0, 24, 12, 24],
        borderRadius: [15, 15, 15, 15],
        background: '#E8394E99',
      }),
      new MLDraftLink({
        isFilled: false,
        href: 'https://vk.com/audio',
        linkType: 'third-party' as const,
        title: 'Слушай в VK',
        textAlign: 'center',
        fontSize: 24,
        fontWeight: 400,
        fontFamily: 'Inter, sans-serif',
        color: '#fff',
        padding: [24, 24, 24, 24],
        margin: [0, 24, 12, 24],
        borderRadius: [15, 15, 15, 15],
        background: '#E8394E99',
      }),
      new MLDraftLink({
        isFilled: false,
        href: 'https://open.spotify.com/',
        linkType: 'third-party' as const,
        title: 'Слушай в Spotify',
        textAlign: 'center',
        fontSize: 24,
        fontWeight: 400,
        fontFamily: 'Inter, sans-serif',
        color: '#fff',
        padding: [24, 24, 24, 24],
        margin: [0, 24, 12, 24],
        borderRadius: [15, 15, 15, 15],
        background: '#E8394E99',
      }),
      new MLDraftLink({
        isFilled: false,
        href: 'https://music.yandex.by/home',
        linkType: 'third-party' as const,
        title: 'Слушай в Яндекс',
        textAlign: 'center',
        fontSize: 24,
        fontWeight: 400,
        fontFamily: 'Inter, sans-serif',
        color: '#FFFBFB',
        padding: [24, 24, 24, 24],
        margin: [0, 24, 12, 24],
        borderRadius: [50, 15, 15, 50],
        background: '#E8394E99',
      }),
      new MLDraftSocial({
        isFilled: false,
        links: [
          'https://vk.com',
          'https://youtube.com',
          'https://www.tiktok.com/',
          'https://instagram.com',
        ],
        linkTypes: [
          SocialNetwork.VK,
          SocialNetwork.YOUTUBE,
          SocialNetwork.TIKTOK,
          SocialNetwork.INSTAGRAM,
        ],
        size: 65,
        margin: [55, 55, 55, 55],
        padding: [0, 17, 0, 17],
      }),
    ],
  },
  {
    name: 'Ulyashchinkov',
    background: '#0000',
    outerBackground: '#0000',
    maxWidth: 480,
    blocks: [
      // Ulyashchinkov
      new MLDraftLogo({
        isFilled: true,
        logo: null,
        size: 154,
        padding: [6, 0, 6, 0],
      }),
      new MLDraftText({
        isFilled: false,
        text: 'Ульященков Сергей',
        fontSize: 24,
        fontWeight: 400,
        fontFamily: 'Inter, sans-serif',
        color: '#fff',
        textAlign: 'center',
        padding: [16, 24, 16, 24],
        background: '#E8A23999',
      }),
      new MLDraftText({
        isFilled: false,
        text: 'Уже больше 20лет я помогаю компаниям и стартапам',
        fontSize: 16,
        fontWeight: 400,
        fontFamily: 'Inter, sans-serif',
        textAlign: 'center',
        color: '#fff',
        padding: [12, 24, 12, 24],
        background: '#E8A23999',
      }),
      new MLDraftImage({
        isFilled: true,
        image: null,
        imgPosition: 'bottom',
        textPosition: 'inside',
        padding: [39, 0, 20, 0],
      }),
      new MLDraftSocial({
        isFilled: false,
        links: ['https://zen.yandex.ru/', 'https://vk.com', 'https://t.me'],
        linkTypes: [
          SocialService.DZEN,
          SocialNetwork.VK,
          SocialService.ARTOFDIGITAL,
          SocialNetwork.TELEGRAM,
        ],
        size: 65,
        margin: [35, 35, 35, 35],
        padding: [0, 17, 0, 17],
      }),
    ],
  },
  {
    name: 'deadp47',
    background: '#0000',
    outerBackground: '#0000',
    maxWidth: 480,
    blocks: [
      // deadp47
      new MLDraftLogo({
        isFilled: true,
        logo: null,
        size: 154,
        padding: [36, 0, 6, 0],
      }),
      new MLDraftText({
        isFilled: false,
        text: 'DeadP47',
        fontSize: 22,
        fontWeight: 400,
        fontFamily: 'Inter, sans-serif',
        color: '#fff',
        textAlign: 'center',
        margin: [0, 0, 0, 0],
        padding: [22, 24, 12, 24],
        background: 'rgba(232, 57, 204, 0.6)',
      }),
      new MLDraftText({
        isFilled: false,
        text: 'Пивные аниме стримы по пятницам',
        fontSize: 18,
        fontWeight: 400,
        fontFamily: 'Inter, sans-serif',
        textAlign: 'center',
        color: '#fff',
        margin: [0, 0, 0, 0],
        padding: [12, 24, 32, 24],
        background: 'rgba(232, 57, 204, 0.6)',
      }),
      new MLDraftSocial({
        isFilled: false,
        links: ['https://vk.com', 'https://youtube.com', 'https://t.me', 'https://instagram.com'],
        linkTypes: [
          SocialNetwork.VK,
          SocialNetwork.YOUTUBE,
          SocialNetwork.TELEGRAM,
          SocialNetwork.INSTAGRAM,
        ],
        size: 65,
        margin: [39, 0, 33, 0],
        padding: [0, 17, 0, 17],
      }),
      new MLDraftSocial({
        isFilled: false,
        links: [
          'https://www.tiktok.com/',
          'https://wasd.tv/',
          'https://discord.com/',
          'https://www.twitch.tv/',
        ],
        linkTypes: [
          SocialNetwork.TIKTOK,
          SocialService.WASD,
          SocialService.DISCORD,
          SocialService.TWITCH,
        ],
        size: 65,
        margin: [0, 0, 0, 0],
        padding: [0, 17, 0, 17],
      }),
      new MLDraftText({
        isFilled: true,
        text: 'На жизнь за границей',
        textAlign: 'center',
        fontSize: 24,
        fontWeight: 400,
        fontFamily: 'Inter, sans-serif',
        color: '#FEF9F9',
        margin: [18, 0, 0, 0],
        padding: [0, 0, 0, 0],
      }),
      new MLDraftText({
        isFilled: true,
        text: '1',
        color: 'rgba(254, 21, 217, 0)',
        background: 'rgba(254, 21, 217, 0.85)',
        margin: [0, 92, 0, 0],
        padding: [8, 0, 8, 0],
      }),
      new MLDraftText({
        isFilled: true,
        text: '1',
        color: 'rgba(102, 82, 99, 0)',
        background: 'rgba(102, 82, 99, 0.6)',
        margin: [-35.2, 0, 0, 283],
        padding: [8, 0, 8, 0],
      }),
      new MLDraftText({
        isFilled: true,
        text: '0',
        fontSize: 22,
        fontWeight: 400,
        fontFamily: 'Inter, sans-serif',
        color: '#FEF9F9',
        margin: [0, 0, 0, 0],
        padding: [0, 0, 0, 0],
      }),
      new MLDraftText({
        isFilled: true,
        text: '200 000',
        color: '#FEF9F9',
        fontSize: 22,
        fontWeight: 400,
        fontFamily: 'Inter, sans-serif',
        margin: [-27, 0, 0, 280],
        padding: [0, 0, 0, 0],
      }),
      new MLDraftSocial({
        isFilled: false,
        links: ['https://boosty.to/', 'https://www.patreon.com/', 'https://www.twitch.tv/'],
        linkTypes: [SocialService.BOOSTY, SocialService.PATREON, SocialService.DONATIONALERTS],
        size: 65,
        margin: [5, 5, 5, 5],
        padding: [0, 17, 0, 17],
      }),
    ],
  },
  {
    name: 'wearearsenal',
    background: '#0000',
    outerBackground: '#0000',
    maxWidth: 480,
    blocks: [
      // wearearsenal
      /*       new MLDraftLogo({
        isFilled: !!avatar,
        logo,
        size: 110,
        margin: [0, 0, 24, 0],
        padding: [16, 0, 0, 0],
      }), */
      new MLDraftImage({
        isFilled: true,
        image: 'https://i.ibb.co/chLtmc0/image-0-0.png',
        imgPosition: 'bottom',
        textPosition: 'inside',
        margin: [0, 100, 0, 100],
        padding: [0, 24, 24, 24],
      }),
      new MLDraftText({
        isFilled: false,
        text: 'Арсенал - We Are The Arsenal',
        fontWeight: 400,
        fontSize: 36,
        fontFamily: 'Inter, sans-serif',
        color: '#fff',
        textAlign: 'center',
        textShadow: ['2px 1px 2px #0005'],
        margin: [0, 0, 0, 0],
        padding: [12, 24, 12, 24],
      }),
      new MLDraftText({
        isFilled: false,
        text: `Авторский проект про Арсенал\n\nПодкасты, новости и переводы`,
        fontWeight: 700,
        fontSize: 16,
        fontFamily: 'Inter, sans-serif',
        textAlign: 'center',
        color: '#fff',
        textShadow: ['2px 1px 2px #0005'],
        margin: [0, 0, 0, 0],
        padding: [8, 24, 8, 24],
      }),
      new MLDraftLink({
        isFilled: false,
        href: 'https://www.youtube.com/channel/UCJA2ybVcB9DjOr66E8Lpp-g',
        linkType: 'third-party' as const,
        title: 'Наш Youtube канал',
        textAlign: 'center',
        image: 'https://i.ibb.co/bLKpx6p/link-0-1.png',
        fontWeight: 700,
        fontSize: 20,
        fontFamily: 'Inter, sans-serif',
        color: '#fff',
        padding: [24, 0, 24, 56],
        margin: [33, 24, 28, 24],
        borderRadius: [50, 15, 15, 50],
        background: 'rgba(133, 164, 204, 0.6)',
      }),
      new MLDraftLink({
        isFilled: false,
        href: 'https://vk.com/wearethearsenal',
        linkType: 'third-party' as const,
        title: 'Главный фанатский паблик в ВК',
        textAlign: 'center',
        image: 'https://i.ibb.co/PTYQCK9/link-0-1.png',
        fontWeight: 700,
        fontSize: 20,
        fontFamily: 'Inter, sans-serif',
        color: '#fff',
        padding: [10, 0, 10, 56],
        margin: [0, 24, 28, 24],
        borderRadius: [50, 15, 15, 50],
        background: 'rgba(133, 164, 204, 0.6)',
      }),
      new MLDraftLink({
        isFilled: false,
        href: 'https://boosty.to/asan1886',
        linkType: 'third-party' as const,
        title: 'Поддержка проекта',
        textAlign: 'center',
        image: 'https://i.ibb.co/2M1ndWN/link-0-1.png',
        imageMargin: [1, 1, 1, 1],
        fontWeight: 700,
        fontSize: 20,
        fontFamily: 'Inter, sans-serif',
        color: '#fff',
        padding: [24, 0, 24, 56],
        margin: [0, 24, 28, 24],
        borderRadius: [50, 15, 15, 50],
        background: 'rgba(133, 164, 204, 0.6)',
      }),
      new MLDraftLink({
        isFilled: false,
        href: 'https://t.me/abibyllaev',
        linkType: 'third-party' as const,
        title: 'Канал про Арсенал',
        textAlign: 'center',
        image: 'https://i.ibb.co/XY98sWp/link-0-1.png',
        fontWeight: 700,
        fontSize: 20,
        fontFamily: 'Inter, sans-serif',
        color: '#fff',
        padding: [24, 0, 24, 56],
        margin: [0, 24, 28, 24],
        borderRadius: [50, 15, 15, 50],
        background: 'rgba(133, 164, 204, 0.6)',
      }),
      new MLDraftLink({
        isFilled: false,
        href: 'https://vk.com/arsenalkurilka',
        linkType: 'third-party' as const,
        title: 'Мемный паблик',
        textAlign: 'center',
        image: 'https://i.ibb.co/PTYQCK9/link-0-1.png',
        fontWeight: 700,
        fontSize: 20,
        fontFamily: 'Inter, sans-serif',
        color: '#fff',
        padding: [24, 0, 24, 56],
        margin: [0, 24, 28, 24],
        borderRadius: [50, 15, 15, 50],
        background: 'rgba(133, 164, 204, 0.6)',
      }),
    ],
  },
  {
    name: 'ROMA-FOBI',
    background: 'url(https://i.ibb.co/j88XWL8/background-9999-0.png)',
    outerBackground: 'url(https://i.ibb.co/j88XWL8/background-9999-0.png)',
    maxWidth: 480,
    blocks: [
      // ROMA-FOBI
      new MLDraftLogo({
        isFilled: true,
        logo: 'https://i.ibb.co/cbC6yrg/image-0-0.png',
        size: 161,
        margin: [0, 0, 24, 0],
      }),
      new MLDraftText({
        isFilled: false,
        text: 'ROMA FOBI',
        fontWeight: 700,
        fontSize: 24,
        fontFamily: 'Caveat, sans-serif',
        textAlign: 'center',
        textShadow: ['2px 1px 2px #0007'],
        padding: [15, 24, 15, 24],
        borderRadius: [20, 20, 20, 20],
        margin: [0, 76, 32, 76],
        background: 'rgba(244, 244, 244, 1)',
      }),
      new MLDraftCarousel({
        isFilled: false,
        dots: true,
        arrows: false,
        images: [sliderImage1, sliderImage2, sliderImage3],
        interval: 5000,
        margin: [0, 24, 24, 24],
      }),
      new MLDraftLink({
        isFilled: false,
        href: 'http://www.tiktok.com/@roma_fobi',
        linkType: 'third-party' as const,
        title: 'TIK TOK',
        image: 'https://i.ibb.co/zm7cZj3/logo-0-1.png',
        imageMargin: [14, 14, 14, 14],
        textAlign: 'center',
        fontWeight: 700,
        fontSize: 22,
        fontFamily: 'Caveat, sans-serif',
        textShadow: ['2px 1px 2px #0007'],
        padding: [15, 24, 15, 24],
        borderRadius: [20, 20, 20, 20],
        margin: [0, 76, 32, 76],
        background: 'rgba(244, 244, 244, 1)',
      }),
      new MLDraftSocial({
        isFilled: false,
        links: [
          'mailto://notofficial777@mail.ru',
          'http://www.tiktok.com/@roma_fobi',
          'https://vc.ru/u/1137112-roma-fobi',
          'https://www.instagram.com/roma_fobi',
        ],
        linkTypes: [
          SocialService.GMAIL,
          SocialNetwork.TIKTOK,
          SocialService.VC,
          SocialNetwork.INSTAGRAM,
        ],
        size: 65,
        margin: [0, 0, 0, 0],
        padding: [0, 17, 0, 17],
      }),
    ],
  },
];
